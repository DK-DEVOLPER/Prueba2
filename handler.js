		...
		for (let name in plugins) {
			...
			if ((usedPrefix = (match[0] || '')[0])) {
				...
				if (!isPrems && plugin.limit && db.data.users[m.sender].limit < plugin.limit * 1) {
					this.reply(m.chat, `Se te acabaron los *limites* , para comprar mas usa *${usedPrefix}buy*`, m)
...
export async function groupsUpdate(groupsUpdate) {
	...
	for (const groupUpdate of groupsUpdate) {
		...
		if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || Connection.conn.sDesc || '```La descripción cambió a```\n@desc').replace('@desc', groupUpdate.desc)
		if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || Connection.conn.sSubject || '```El nombre del grupo cambio a```\n@subject').replace('@subject', groupUpdate.subject)
		if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || Connection.conn.sIcon || '```El icono del grupo fue actualizado```').replace('@icon', groupUpdate.icon)
		if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || Connection.conn.sRevoke || '```El link del grupo cambió a```\n@revoke').replace('@revoke', groupUpdate.revoke)
		if (!text) continue
...
export async function deleteUpdate(message) {
	...
	if (Array.isArray(message.keys) && message.keys.length > 0) {
		const tasks = await Promise.allSettled(message.keys.map(async (key) => {
			...
			await this.reply(key.remoteJid, `@${participant.split`@`[0]} ha eliminado el mensaje\nPara desactivar esta función,\n*.off antidelete*`, msg, { mentions: [participant] })
...
global.dfail = (type, m, conn) => {
	let msg = {
		rowner: `*「SOLO KENISAWA」*`,
		owner: `*「SOLO PROPIETARIO」*`,
		mods: `*「SOLO DEV / MODS」*`,
		premium: `*「SOLO PREMIUMS」*`,
		group: `*「SOLO GRUPOS」*`,
		private: `*「SOLO CHAT PRIVADO」*`,
		admin: `*「SOLO ADMINS」*`,
		nsfw: `[ *MODO NSFW INACTIVO* ]`,
		game: '```「 ACTIVA EL MODO JUEGO! 」```',
		botAdmin: `*「EL BOT NO ES ADMIN」*`,
		unreg: 'Regístrese para utilizar esta función escribiendo:\n\n*#reg nombre.edad*\n\nEjemplo: *#reg Mauro.18*',
