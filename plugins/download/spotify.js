...
let handler = async (m, { conn, text, usedPrefix, command, __dirname }) => {
	if (!text) throw `Ej: ${usedPrefix + command} Louca encubadora`
	if (isUrl(text)) {
		if (/\/track\//.test(text)) {
			try {
				let res = await SpottyDL.getTrack(text)
				if (typeof res !== 'object') throw 'url de Spotify invalida'
			...
			anu = anu.tracks
			if (typeof anu !== 'object' || anu.length == 0) throw 'url de album inválida '
			let txt = `*Encontre ${anu.length} Resultados*`
			for (let x of anu) {
				txt += `\n\n*Nombre :* ${x.name}\n`
				+ `*Nunero de pista :* ${x.trackNumber}\n`
		...
		if (anu.length == 0) throw 'error de api.'
