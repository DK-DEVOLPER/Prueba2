...
let handler = async (m, { conn, usedPrefix: _p, __dirname, isPrems }) => {
	try {
		...
		let _text = [
			before.replace(': *%limit', `${isPrems ? ': *Infinito' : ': *%limit'}`),
		...
		await conn.sendFThumb(m.chat, db.data.datas.maingroupname, text.trim(), "https://wallpapersmug.com/large/792616/cyberpunk-edgerunners-lucy-fan-art.jpg", db.data.datas.linkgc, m)
	} catch (e) {
		console.log(e)
	}
}
