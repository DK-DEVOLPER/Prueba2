...
let handler = async (m, { conn, text, participants }) => {
	if (db.data.settings[conn.user.jid].restrict) throw `[ MODO REINSTRINGIDO ]`
	let who, not = [], users = []
	...
	if (who.length == 0) throw `A quien desea agregar ?`
	...
	if (users.length == 0) throw `@${not.join(', @')} no esta registrado en WhatsApp`
	if (not.length > 0) m.reply(`@${not.join(', @')} no esta registrado en WhatsApp`)
	...
	for (let i of users) {
		...
		for (let x of res) {
			if (x.status == '403') {
				...
				await conn.reply(m.chat, `No se pudo agregar a @${x.jid.split('@')[0]}, enviando invitación...`, m, { mentions: [x.jid] })
				...
				await conn.reply(m.chat, `No puedes agregar a @${x.jid.split('@')[0]} porque dejó el grupo recientemente. Vuelve a intentarlo más tarde.`, m, { mentions: [x.jid] })
			} else if (x.status == '409') {
				await conn.reply(m.chat, `@${x.jid.split('@')[0]} ya esta en el grupo.`, m, { mentions: [x.jid] })
			} else {
