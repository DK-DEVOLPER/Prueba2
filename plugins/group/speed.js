...
let handler = async (m, { conn }) => {
	...
	let speed = neww - old
	let txt = `Velocidad de respuesta ${speed.toFixed(4)} segundos\n\n`
	txt += `Rutina :\n*${runtime(process.uptime())}*\n`
	txt += `OS Uptime :\n*${runtime(os.uptime())}*\n\n`
	txt += `💬 Estado :\n- *${groupsIn.length < groups.length ? groups.length : groupsIn.length}* Grupos\n`
	txt += `- *${groups.length}* Grupos unidos\n`
	txt += `- *${groupsIn.length < groups.length ? 0 : groupsIn.length - groups.length}* Grupos abandonados\n`
	txt += `- *${chats.length - groupsIn.length}* Pv's\n`
