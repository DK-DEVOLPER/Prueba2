...
let handler = async (m, { conn, command, usedPrefix }) => {
	...
	if (new Date - user.lastfishing <= cooldown) return m.reply(`📍 Has pescado, entonces puedes hacerlo en  . . .\n🕖 *${((user.lastfishing + cooldown) - new Date()).toTimeString()}*`)
	if (user.fishingrod == 0) return m.reply(`Usa *${usedPrefix}craft* para conseguir una caña de pesca.\n\nTienes :\n━ 🎣 ${user.fishingrod} Caña de pesca`)
	let mancing = [
 ...
 *🦐 = [ ${mancing[12].ikan} ]*`
	...
	setTimeout(() => {
		m.reply('_estas pescando..._')
	}, 0)
	user.lastfishing = new Date * 1
	user.mancingcount += 1
}

handler.menufun = ['pescar']
handler.tagsfun = ['rpg']
